...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Snackbar.make(view, "Произошло событие", Snackbar.LENGTH_LONG)
        ...
        ftrans.replace(R.id.content_main, new Calculate());
        ftrans.commit();
        setTitle("Вычисления");
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        int id = item.getItemId();
        AppBarLayout abl = (AppBarLayout) findViewById(R.id.appbar_calc);

        FragmentManager fragmentManager = getSupportFragmentManager();
        FragmentTransaction ftrans = fragmentManager.beginTransaction();

        if (id == R.id.nav_calc) {
            ftrans.replace(R.id.content_main, new Calculate()).commit();
            abl.setElevation(0);
        } else if (id == R.id.nav_cypher) {
            abl.setElevation(14);
            ftrans.replace(R.id.content_main, new Encrypting()).commit();
        } else if (id == R.id.nav_uncypher) {
            abl.setElevation(14);
